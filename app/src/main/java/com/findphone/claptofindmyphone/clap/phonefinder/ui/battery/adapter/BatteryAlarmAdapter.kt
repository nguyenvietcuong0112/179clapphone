package com.findphone.claptofindmyphone.clap.phonefinder.ui.battery.adapter
...
import com.findphone.claptofindmyphone.clap.phonefinder.model.AlarmRepeatTimes
...
import com.findphone.claptofindmyphone.clap.phonefinder.model.NotificationSound
...
class BatteryAlarmAdapter(private val alarms: List<BatteryAlarmSettings>) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.switchAlarm.setOnCheckedChangeListener { _, isChecked ->
            ...
            if(alarm.alarmType == AlarmType.BATTERY_LOW && !isChecked) {
                val intent = Intent("com.finder.clap.findphone.whistle.byclapping.ACTION_STOP_LOW_BATTERY_ALARM")
                holder.view.context.sendBroadcast(intent)
            }

            if(alarm.alarmType == AlarmType.BATTERY_FULL && !isChecked) {
                val intent = Intent("com.finder.clap.findphone.whistle.byclapping.ACTION_STOP_FULL_BATTERY_ALARM")
                holder.view.context.sendBroadcast(intent)
            }
        }
    ...
    private fun selectRepeatOption(
        ...
        selectedOption.backgroundTintList = ContextCompat.getColorStateList(context, R.color.colorButtonSelect)

        for (option in otherOptions) {
            option.backgroundTintList = ContextCompat.getColorStateList(context, R.color.colorButtonUnselect)
        }
    }
